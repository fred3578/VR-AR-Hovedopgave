    ...
    public class CloudAnchorsExampleController : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            if (Application.platform != RuntimePlatform.IPhonePlayer)
            {
                ...
                {   
                    ...
                    if (hitInfo.transform.tag == "BrickBlackTag" || hitInfo.transform.tag == "BrickWhiteTag")
                    {
                        ...
                        Managers.GetComponent<BrickController1>().MoveBrick(hitInfo);
                        Managers.GetComponent<BrickController1>().OnGUI();
                    }
                }
            ...
            if (m_LastPlacedAnchor != null)
            {
                ...
                if (_CanPlaceStars() && GameObject.FindGameObjectsWithTag("BrickBlackTag").Length <= 5 )
                {
                    _InstantiateBlackBrick();
                    
                    
                }
                if (_CanPlaceStars() && GameObject.FindGameObjectsWithTag("BrickBlackTag").Length >= 5 && GameObject.FindGameObjectsWithTag("BrickWhiteTag").Length <= 5)
                {
                    _InstantiateWhiteBrick();
        ...
        /// </summary>
        private void _InstantiateBlackBrick()
        {
            BrickID = "BrickBlack" + BrickBlackResultat;
            // Star must be spawned in the server so a networking Command is used.
            BrickBlackResultat += BrickBlackResultat;
            GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>()
                      .CmdSpawnBlackBrick(m_LastPlacedAnchor.transform.position, m_LastPlacedAnchor.transform.rotation, BrickID);
        }

        private void _InstantiateWhiteBrick()
        {
            // Star must be spawned in the server so a networking Command is used.
            GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>()
                .CmdSpawnWhiteBrick(m_LastPlacedAnchor.transform.position, m_LastPlacedAnchor.transform.rotation);
        }
